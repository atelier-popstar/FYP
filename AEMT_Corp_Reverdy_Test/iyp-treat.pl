...
# Last Modified: Tue Aug 14 15:47:56 2012 (vogel) 
...
if (defined $opt_p) {$punct=$opt_p;}else {$punct=0}; 
if (defined $opt_c) {$case=$opt_c;}else {$case=1}; 

if (defined $opt_o) {$o=$opt_o;}else {$o="$i.iy.p$punct.c$case";}; 
if (defined $opt_l) {$l=$opt_l;}else {$l="$o.iy";}; 
...
sub ignorethis {
   if (lteall($dk,($dn,$dd,$dy,$dg))){
	if ($ki < $kindex){

	my $spnow = "k";
	my $words = speech($k{$ki});
	my $contrib = transcriptindex($k{$ki})."\t".speechstarttime($k{$ki})."\t".speechstoptime($k{$ki});
...
#	display($spnow,%registers);
	...
	$overlaps = overlap($spnow,$kstart,$kstop,%times);
	$times{$spnow} = "$kstart\t$kstop";
	print FILE "$overlaps\t$day\t$reality\n";
...
while (1 < 0)
{
    $start = speechstarttime($k{$ki});
    $stop = speechstoptime($k{$ki});
    $spoken = speech($k{$ki});
    print "$ki\t$start\t$stop\t$spoken\n";
    $ki++;
}
